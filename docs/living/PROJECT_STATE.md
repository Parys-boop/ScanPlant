{
  ...
  "next_action": "Checkpoint factual P03-R5/UFV28M: no WSL normal, expected_revision == revision == HEAD == upstream == cfbfb6f67b0f45f11b06aab55716139f3cc89dfe, branch bm/v2-p03 e 0/0 antes da execução. IPv4/IPv6 loopback, rg 15.1.0, SDK 8.0.424, runtimes 8.0.30, tool restore/list dotnet-stryker 4.16.0, launcher não mutacional, restore/build offline do MutationHarness e teste final 23/23 passaram. A falha posterior foi exclusivamente a sintaxe inválida rg -Eq no gate de resumo: -E foi interpretado como --encoding e q causou unknown encoding: q; exit final 95. campaign_count=0, campaign_authorized=false e campaign_executed=false; não há mutation-report.json, campanha ou resultado de mutação. P01 permanece blocked-terminal, P03 permanece bloqueado e U-008 não foi concedida nem registrada. Após este checkpoint publicado e HEAD==upstream 0/0 com árvore limpa, executar somente o novo launcher corrigido no WSL normal sob autorização humana separada; não iniciar campanha."
}
